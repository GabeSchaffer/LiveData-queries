...
FROM (
	SELECT XMLElement("PlannerCase",
		XMLAttributes(CLEANUP_DURATION AS "turnoverTime",
			(SELECT OE_FIELD_DISPLAY_VALUE FROM SCH_EVENT_DETAIL WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID AND OE_FIELD_ID = {OE_FIELD_EDC} AND VERSION_DT_TM = NULL_DT_TM) AS "edc",
			(SELECT OE_FIELD_DISPLAY_VALUE FROM SCH_EVENT_DETAIL WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID AND OE_FIELD_ID = {OE_FIELD_GRAVIDA} AND VERSION_DT_TM = NULL_DT_TM) AS "gravida",
			(SELECT OE_FIELD_DISPLAY_VALUE FROM SCH_EVENT_DETAIL WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID AND OE_FIELD_ID = {OE_FIELD_PARA} AND VERSION_DT_TM = NULL_DT_TM) AS "para",
			(SELECT APPT_TYPE_CD FROM SCH_EVENT WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID) AS "apptTypeId",
			NULLIF((COALESCE(        (SELECT TO_CHAR(LONG_TEXT) FROM LONG_TEXT WHERE LONG_TEXT_ID = (SELECT TEXT_ID FROM SCH_EVENT_COMM WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID AND SUB_TEXT_MEANING = 'SURGPUBLIC' AND VERSION_DT_TM = NULL_DT_TM)),        (SELECT OE_FIELD_DISPLAY_VALUE FROM SCH_EVENT_DETAIL WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID AND OE_FIELD_ID = {OE_FIELD_COMMENT} AND VERSION_DT_TM = NULL_DT_TM))), ' ') AS "publicComment",
			...
			(SELECT OE_FIELD_DISPLAY_VALUE FROM SCH_EVENT_DETAIL WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID AND OE_FIELD_ID = {OE_FIELD_GBS_STATUS} AND VERSION_DT_TM = NULL_DT_TM) AS "gbsStatus",
		...
		(SELECT XMLAgg(XMLElement("SurgicalCase",
			XMLAttributes((SELECT DISPLAY FROM CODE_VALUE WHERE CODE_VALUE = SCHED_PAT_TYPE_CD) AS "patientType",
				...
				(SELECT DECODE(pd.REC_VER_ID, 0, 0, 1) FROM PERIOPERATIVE_DOCUMENT pd WHERE pd.SURG_CASE_ID = PLANNER_SURGICAL_CASE.SURG_CASE_ID AND pd.DOC_TYPE_CD IN ({DOC_TYPE_CD_INTRAOP})) AS "isFinalized",
		...
		(SELECT XMLAgg(XMLElement("Encounter",
			...
			(SELECT XMLAgg(XMLElement("PlannerPatient",
				...
				WHERE PERSON_ALIAS.PERSON_ID = PERSON.PERSON_ID)))
			FROM (PERSON) PERSON
	...
	WHERE ACTIVE_IND = 1 AND BEG_DT_TM BETWEEN {startDate} AND {endDate} AND SCH_EVENT_ID != 0 AND REAL_ENCNTR_ID IS NOT NULL AND SCHEDULE_SEQ = (SELECT MAX(SCHEDULE_SEQ) FROM SCH_APPT WHERE SCH_EVENT_ID = APPT_CASE.SCH_EVENT_ID) AND RESOURCE_CD IN (SELECT RESOURCE_CD FROM SCH_BOOK_LIST WHERE APPT_BOOK_ID IN ({APPT_BOOKS}))
	ORDER BY BEG_DT_TM)