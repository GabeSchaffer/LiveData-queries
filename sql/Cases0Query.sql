...
FROM (
	SELECT XMLElement("Case",
		XMLAttributes((SELECT DISPLAY FROM CODE_VALUE WHERE CODE_VALUE = SCHED_PAT_TYPE_CD) AS "patientType",
			...
			(SELECT DECODE(pd.REC_VER_ID, 0, 0, 1) FROM PERIOPERATIVE_DOCUMENT pd WHERE pd.SURG_CASE_ID = SURGICAL_CASE.SURG_CASE_ID AND pd.DOC_TYPE_CD IN ({DOC_TYPE_CD_INTRAOP})) AS "isFinalized",
			...
			/* ALIASPOOL ECHACCOUNTNUMBER */ (SELECT MAX(ALIAS) FROM ENCNTR_ALIAS ea WHERE ea.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID AND ALIAS_POOL_CD = {ALIAS_POOL_CD_ACCTNUM} AND ACTIVE_IND = 1) AS "acctNum",
			...
			/* ALIASPOOL ECHMRN */ (SELECT MAX(ALIAS) FROM ENCNTR_ALIAS ea WHERE ea.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID AND ALIAS_POOL_CD = {ALIAS_POOL_CD_MRN} AND ACTIVE_IND = 1) AS "mrn"),
		...
		WHERE ACTIVE_IND = 1 AND (SELECT DOC_TYPE_CD FROM PERIOPERATIVE_DOCUMENT pd JOIN SEGMENT_HEADER sh ON pd.PERIOP_DOC_ID = sh.PERIOP_DOC_ID WHERE sh.SEGMENT_HEADER_ID = CASE_ATTENDANCE.SEGMENT_HEADER_ID) IN ({DOC_TYPE_CD_INTRAOP}) AND CASE_ATTENDANCE.SURG_CASE_ID = SURGICAL_CASE.SURG_CASE_ID),
		...
		WHERE EVENT_CD = {EVENT_CD_HEIGHT} AND RECORD_STATUS_CD = 188 AND REGEXP_LIKE(RESULT_VAL, '^[0-9.]+$') AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND HEIGHT.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		WHERE EVENT_CD in ({EVENT_CD_WEIGHT}) AND RECORD_STATUS_CD = 188 AND REGEXP_LIKE(RESULT_VAL, '^[0-9.]+$') AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND WEIGHT.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		WHERE EVENT_CD in ({EVENT_CD_TQT}) AND RECORD_STATUS_CD = 188 AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND TQT.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		WHERE EVENT_CD in ({EVENT_CD_SPECIMEN}) AND RECORD_STATUS_CD = 188 AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND SPECIMEN.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
	...
	WHERE ACTIVE_IND = 1 AND DEPT_CD = {DEPT_CD} AND SCHED_SURG_AREA_CD NOT IN ({SURG_AREA_CD_EXCLUDE}) AND SCHED_START_DT_TM BETWEEN {startDate} AND {endDate})