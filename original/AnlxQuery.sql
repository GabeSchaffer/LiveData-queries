...
FROM (
	SELECT XMLElement("Case",
		XMLAttributes((SELECT DISPLAY FROM CODE_VALUE WHERE CODE_VALUE = SCHED_PAT_TYPE_CD) AS "patientType",
			...
			(SELECT DECODE(pd.REC_VER_ID, 0, 0, 1) FROM PERIOPERATIVE_DOCUMENT pd WHERE pd.SURG_CASE_ID = SURGICAL_CASE.SURG_CASE_ID AND pd.DOC_TYPE_CD IN (45065182, 45065477, 49258577)) AS "isFinalized",
			...
			/* ALIASPOOL ECHACCOUNTNUMBER */ (SELECT MAX(ALIAS) FROM ENCNTR_ALIAS ea WHERE ea.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID AND ALIAS_POOL_CD = 49211702 AND ACTIVE_IND = 1) AS "acctNum",
			...
			/* ALIASPOOL ECHMRN */ (SELECT MAX(ALIAS) FROM ENCNTR_ALIAS ea WHERE ea.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID AND ALIAS_POOL_CD = 49320274 AND ACTIVE_IND = 1) AS "mrn"),
		...
		WHERE ACTIVE_IND = 1 AND (SELECT DOC_TYPE_CD FROM PERIOPERATIVE_DOCUMENT pd JOIN SEGMENT_HEADER sh ON pd.PERIOP_DOC_ID = sh.PERIOP_DOC_ID WHERE sh.SEGMENT_HEADER_ID = CASE_ATTENDANCE.SEGMENT_HEADER_ID) IN (45065182, 45065477, 49258577) AND CASE_ATTENDANCE.SURG_CASE_ID = SURGICAL_CASE.SURG_CASE_ID),
		...
		(SELECT XMLAgg(XMLElement("Procedure",
			XMLAttributes((SELECT DISPLAY FROM CODE_VALUE WHERE CODE_VALUE = SCHED_ANESTH_TYPE_CD) AS "schedAnesType",
				...
				(SELECT REPLACE(NAME_FULL_FORMATTED, UNISTR('\0000'), '') FROM PRSNL WHERE PERSON_ID = DECODE(PRIMARY_SURGEON_ID, 0, SCHED_PRIMARY_SURGEON_ID, PRIMARY_SURGEON_ID)) AS "surgeon",
		...
		WHERE EVENT_CD = 2700653 AND RECORD_STATUS_CD = 188 AND REGEXP_LIKE(RESULT_VAL, '^[0-9.]+$') AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND HEIGHT.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		WHERE EVENT_CD in (2700654, 21043169, 12933755) AND RECORD_STATUS_CD = 188 AND REGEXP_LIKE(RESULT_VAL, '^[0-9.]+$') AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND WEIGHT.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		WHERE EVENT_CD in (3017291, 3017292, 3017293, 3017296) AND RECORD_STATUS_CD = 188 AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND TQT.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		WHERE EVENT_CD in (3017281, 8888544, 3017284, 8864640) AND RECORD_STATUS_CD = 188 AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND SPECIMEN.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		WHERE EVENT_CLASS_CD = 232 AND EVENT_TITLE_TEXT != 'IVPARENT' AND EVENT_TITLE_TEXT not like 'Lactated Ringers%' AND RECORD_STATUS_CD = 188 AND SYS_EXTRACT_UTC(SYSTIMESTAMP) between VALID_FROM_DT_TM and VALID_UNTIL_DT_TM AND DRUG.ENCNTR_ID = SURGICAL_CASE.ENCNTR_ID),
		...
		FROM ( SELECT SURG_CASE_ID, REQUEST_QTY, cc.ACTIVE_IND, CLASS_NAME, SHORT_DESC, STOCK_NBR FROM CASE_CART_PICK_LIST cc  JOIN MM_OMF_ITEM_MASTER mm on mm.ITEM_MASTER_ID = cc.ITEM_ID) EQUIPMENT
		WHERE ACTIVE_IND = 1 AND CLASS_NAME IN ('01 Equipment', '03 Instruments') AND EQUIPMENT.SURG_CASE_ID = SURGICAL_CASE.SURG_CASE_ID)).GetClobVal()
	FROM (SURGICAL_CASE) SURGICAL_CASE
	WHERE ACTIVE_IND = 1 AND DEPT_CD = 38626629 AND SCHED_SURG_AREA_CD NOT IN (38624451) AND SCHED_START_DT_TM BETWEEN DATE '2023-08-27' AND DATE '2023-10-27')